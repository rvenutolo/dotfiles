...
function ___alias_completion() {
  ...
  command rm -f "/tmp/${namespace}-*.tmp" # preliminary cleanup
  local tmp_file
  tmp_file="$(mktemp "/tmp/${namespace}-${RANDOM}XXX.tmp")" || return 1
  local completion_loader
  completion_loader="$(complete -p -D 2> /dev/null | sed -rne 's/.* -F ([^ ]*).*/\1/p')"
